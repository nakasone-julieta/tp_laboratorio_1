...
int utn_rellenarArrayInt(int array[], int len, char* pMensaje, char* pMensajeError, int min, int max, int reintentos)
{
	int retornoFuncion = -1;
	int bufferInt;
	int contadorReintentos =0;
	if(array != NULL && len >0 && pMensaje != NULL, min <= max && reintentos <= 0)
	{
		for(int i=0; i<len; i++)
		{
			printf ("%s", pMensaje);
			...
			else if (contadorReintentos <= reintentos)
			{
				printf("%s", pMensajeError);
				retornoFuncion = -2;//los parámetros recibidos eran correctos pero introdujo valores fuera del rango solicitado
				contadorReintentos ++;
			}
		}
	}
...
int utn_promediarArray(float* pResultado, int array[], int len)//calcular promedio de los valores de Array
{
	...
	if (array != NULL && len >= 0 && pResultado !=NULL)
		{
			for (i=0; i<len; i++)
			{
				retornoFuncion = 0;
