...
static int esNumerica(char* aCadena)
{
	int retorno=1;
	int i=0;
	if(aCadena!=NULL && strlen(aCadena)>0)
	{
		if (aCadena[0]=='-')
		{
			i=1;
		}
		while (aCadena[i] != '\0')//puedo usar un for porq tengo todos los elementos tmb
		{

			if (aCadena [i]<'0' || aCadena[i]>'9')//si el elemento en i sale de los parametros como numero
			{
